...
public class Hand{
    ...
    public Card findACard(String cardShortCode) {
        ...
        int i = 0;
        Card card = null;
        while (!cardFound && i < size()){
            if (handOfCards.get(i).toString().equals(cardShortCode)){
                cardFound = true;
                card = handOfCards.get(i);
            }
            i += 1;
        }
        return card;
    ...
    public void sortHandByFace(){
        ...
        try {
            Collections.sort(handOfCards, Comparator.comparing(Card::getCardFace));
        } catch (Exception e) {
