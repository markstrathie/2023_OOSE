...
public class Sevens extends CardGame{
    ...
    protected boolean playToTable(Card card, Hand[] sevenTableHands){
        ...
        for (Hand hand : sevenTableHands){
            if (!hand.isEmpty() && (hand.getHandOfCards().get(0).getSuit() == card.getSuit())){
                if (hand.getHandOfCards().get(0).getCardFace().getValue() == card.getCardFace().getValue()+1 || hand.getHandOfCards().get(0).getCardFace().getValue() == card.getCardFace().getValue()-1){
                    hand.add(card);
